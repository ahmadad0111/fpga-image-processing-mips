...
INPUT_Display INPUT_Display0 (
  .clka(clk_25mhz), // input clka
  .addra(addr_in), // input [13 : 0] addra
  .douta(dout_in) // output [7 : 0] douta
);		



TOP_median TOP_median0(.clk(clk_100mhz_buf),.clkb(clk_25mhz),.reset(reset),.addr_med(addr_med),.dout_med(dout_med));
IN_TOP IN_TOP00(.clk(clk_100mhz_buf),.clkb(clk_25mhz),.reset(reset),.addr_max(addr_max),.max_out(dout_max),.addr_min(addr_min),.min_out(dout_min));
